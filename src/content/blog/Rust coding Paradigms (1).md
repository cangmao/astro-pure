...
1. 数据类型，规范内存数据的合法性，即对象内容；
   （包括简单数据类型，如整数、浮点数、布尔值等，放入stack中；与复杂数据类型，如数组、元组、结构体、枚举等，不可变部分放入stack，可变部分放入heap中。复杂数据类型可进行匹配与解构。）
2. lifetime，规范内存引用（指针）的有效性，即对象生命周期；
   （对于放入stack的数据引用是默认以copy的方式进行数据传递；放入heap中的数据引用默认以move的方式进行数据传递，同时通过生命周期进行管理。）
3. trait，规范对内存数据的有效操作，即对象行为；
   （具有某一同一特性的数据类型，可以赋予相同的行为。）
